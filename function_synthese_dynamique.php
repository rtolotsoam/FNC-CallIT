...
{
	$str .= " and n.matricule_notation in ( ".$matricule_auditeur." ) ";
}
if($matricule_tlc != 0)
{
	$str .= " and n.matricule in ( ".$matricule_tlc." ) ";
}
if($id_type_appel != 0)
{
	$str .= " and n.id_typologie in ( ".$id_type_appel." ) ";
}
...
function _getCategorieDetailEval($id_projet, $id_client,$id_application,$id_type_traitement,$date_deb_notation,$date_fin_notation,$matricule_auditeur,$matricule_tlc,$id_type_appel)
{
	...
	then 
		...
		case when (id_type_traitement =3 or id_type_traitement =4 or id_client=643) then 1000 else 10 end 
	...
	// echo '_getCategorieDetailEval => <pre>'.$sql.'</pre><br>';
...
order by matricule,id_repartition";
	...
	// echo 'Situation inacceptable => <pre>';
	// echo $sql;
	// echo '</pre><br><br>';
...
function setTableauSynthesePrestation($id_projet, $id_client,$id_application,$id_type_traitement,$date_deb_notation,$date_fin_notation,$matricule_auditeur,$matricule_tlc,$id_type_appel)
{
	...
	$tableau = array();
		...
		from (
		SELECT case when (id_type_traitement = 3 or id_type_traitement = 4) then (sum(somme_produit)/count(id_categorie_grille))/10 else sum(somme_produit)/count(id_categorie_grille) end som
...
function getAllNotation($id_projet, $id_client,$id_application,$id_type_traitement,$date_deb_notation,$date_fin_notation,$matricule_auditeur,$matricule_tlc,$id_type_appel)
{
	...
	$sql = "select distinct n.matricule, 
		case when p.prenompersonnel = '' then 'Inactif' else p.prenompersonnel end prenompersonnel,
		n.date_entretien, 
		n.date_notation, 
		...
		n.id_notation,
		n.point_appui,
		n.point_amelioration,
		n.preconisation,n.note
	from cc_sr_notation n 
...
function getClientById($id_client,$id_application,$matricule_tlc,$matricule_auditeur,$id_type_appel)
{
	...
	$sql2 = "select * from personnel p where p.matricule in ( ".$matricule_tlc." )";
	$query2  = pg_query($sql2) or die(pg_last_error());
	$result2 = pg_fetch_array($query2);
	
	$sql3 = "select * from personnel p where p.matricule in ( ".$matricule_auditeur." ) ";
	$query3  = pg_query($sql3) or die(pg_last_error());
	$result3 = pg_fetch_array($query3);
	
	$sql4 = "select * from cc_sr_typologie t where t.id_typologie in ( ".$id_type_appel." ) ";
...
function getIS($id_projet, $id_client,$id_application,$id_type_traitement,$date_deb_notation,$date_fin_notation,$matricule_auditeur,$matricule_tlc,$id_type_appel)
{
	global $conn;
...
function getDonneesForExport($id_projet,$id_client,$id_application,$ftxt_dtdeb_,$ftxt_dtfin_,$id_type_traitement,$id_type_appel,$matricule_tlc,$matricule_auditeur)
{
	global $conn;
   if($id_type_appel != 0)
   {
...
inner join cc_sr_grille g on g.id_grille= ga.id_grille
inner join cc_sr_categorie_grille cg on cg.id_categorie_grille= g.id_categorie_grille
where ga.id_projet = ".$id_projet." and ga.id_client = ".$id_client." and ga.id_application = ".$id_application
." and cg.id_type_traitement = ".$id_type_traitement." ".$str."
/*and date_notation >= '".$ftxt_dtdeb_."' and date_notation <= '".$ftxt_dtfin_." */
and date_entretien >= '".$ftxt_dtdeb_."' and date_entretien <= '".$ftxt_dtfin_."'";
	
	// echo '<pre>getDonneesForExport => '.$sql.'</pre><br>';
