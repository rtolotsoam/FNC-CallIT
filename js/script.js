...
function do_insert()
{
		...
		function gerer_grille(id_client,id_application,_i){
		     ...
		     tb_show("Gérer les grilles","admin_type.php?height=375&width=850&nomProjet="+nomProjet+"&nomClient="+nomClient+"&nomApplication="+nomApplication+"&id_projet="+IdProjet+"&id_client="+id_client+"&id_application="+id_application);
			 //height:300=>375
		}
		...
		var txtClient = $('#champ_client option:selected').text();
        ...
        function insert_projet(){
               ...
               url:'insert_projet.php',
                    ...
                    success:function(_data2){
							...
							{
								alert("Campagne déjà Enregistré");
							}else{
                                alert("Echec de l'insertion !");
                            }
...
tb_show("Administration de Campagne","gest_questionnaire.php?height=600&width=850");
   ...
   function gestion_droit_acces(){
tb_show("Gestion des droits d'accès","droit.php?height=600&width=705");
   }
   
   function gestion_typologie(){
tb_show("Gestion des typologies","typologie.php?height=500&width=750");
   }
	   ...
	   function modifier_indicateur( IdIndicateur ,_i,id_item)
   {
    ...
    function editer_grille(_i){
      ...
      $("#cache_grille_classement_"+_i).show();
   ...
   function modifier_categorie_grille( id_categorie , _i){
		  ...
		  var id_classement  =$("#cache_grille_classement_"+_i).val();
        ...
        $.post('ajax_script/update_categorie.php',
		  {
			...
			id_classement:id_classement
                  ...
                  $("#cache_grille_libelle_"+_i).hide();
				   $("#cache_grille_classement_"+_i).hide();
  ...
  function get_categorie_ajout(){
   var idTraitement = $("#slct_traitement_ajout").val();
            if(idTraitement==''){
				 ...
				 $("#div_loading_ajout").show();
				 $.post('liste_categorie.php',
				  {
					IdTraitement:idTraitement,
				  },function ( _data ){
					  /****************************/
					  $("#div_loading_ajout").hide();
		...
		function Add( nb_row,max_id1 ){
			...
			"<td class='td_categorie' id='td_test_"+(nb_categorie+1)+"'><input  id='test' size='100%' type='text'/></td>"	+			
		...
		*/	   var id_classement = $("#slct_classement").val();
		                if( id_classement =='0' ){
						     $("#slct_classement").addClass('erreur');
							 return false;
						}
			   ...
			   $.post('insert_categorie.php',
				...
				id_classement:id_classement
				         ...
				         $("#btn_add_categorie").replaceWith( "<img title='Supprimer' onclick='delete_categorie("+(max_id_cat+1)+","+nb_categorie+",1)' class='img_suppr' id='img_suppr_"+(nb_categorie+1)+"' style='width:20px;height:20px;margin:0 0 0 59px;cursor:pointer' src='images/delete.png' /><img title='Afficher les critères' onclick='charge_nouveau_item("+(max_id_cat+1)+",\""+libelle_categorie+"\","+(nb_categorie+1)+")' class='img_suppr' id='img_affiche_"+(nb_categorie+1)+"' style='margin-left:5px;width:20px;height:20px;cursor:pointer' src='images/afficher.png' />" );
		...
		function Delete(){
			...
			par.remove();
				...
				function Active_input( _test )
				{
							 ...
							 $.post('ajax_script/delete_categorie.php',
										  {
											  id_categorie:id_categorie,
												...
												if(_data==1){
												    if( last_id == id_categorie ){
						                              $("#last_id").val( id_categorie-1 );
						                             }
				 ...
				 tb_show("Gérer les notes des grilles","description.php?height=607&width=787&id_projet="+id_projet+"&id_client="+id_client+"&id_application="+id_application+"&nom_projet="+encodeURIComponent(nom_projet)+"&nom_client="+encodeURIComponent(nom_client)+"&nom_application="+encodeURIComponent(nom_application));
				 }
				 
				 
				 function charge_nouveau_item( id_categorie,type ,_k){
				 
				          console.log(_k);
				          if(  typeof _k !== "undefined" ){
						  console.log("ss:"+_k);
				          $('td.td_categorie_'+_k).addClass('green_bg');
				          $('td').not( 'td.td_categorie_'+_k ).removeClass('green_bg').removeClass('class_test');
						  }
				...
				function load_project(){
					var txtClient = $('#champ_client option:selected').text();
					var txtApplication = $('#champ_application option:selected').text();
					var txtApplication = txtApplication.split('-');

					$("#champ_projet").val( txtApplication[0]+" "+txtClient );
				}
				
				function fill_application_2(){
		  
		 var zClient = $("#champ_client").val().split('_');
		...
		var txtClient = $('#champ_client option:selected').text();
              ...
              $.ajax({
                    url:'select_app.php',
                    type:'get',
                    data:{zClient:zClient[0],test_duplication:1},
                     success:function(_data1){
					     _data1 = _data1.split('#');
						 var optApplication = _data1[0];
						 var optCampagne    = _data1[1];
	...
	function admin_duplication_projet(id_client,id_application,_i){
      var id_projet = $("#id_projet"+_i).val();
	  var nom_projet = $("#nom_projet"+_i).val();
	  var nom_client = $("#nom_client"+_i).val();
	  var nom_application = $("#nom_application"+_i).val();
	  var nom_projet = nom_projet.split(" ");
	  var nom_client = nom_client.split(" ");
