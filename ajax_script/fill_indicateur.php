 <?php
	...
	for ( $k = 0 ; $k < @pg_num_rows( $query_traitement ) ; $k++ )
	{
		...
		<thead>";
		$zresult .= "  <tr>";
		$zresult .= "    <th filter='false'>Crit&egrave;re</th>";
		$zresult .= "    <th filter='false'>Indicateur NF</th>";	
		$zresult .= "  </tr>";
		$zresult .= "  </thead><tbody>";
		...
		for ( $i = 0; $i < $iTotal ; $i++ )
		{
			...
			$zresult 	.= "
				<tr>
					...
					<td>".get_taux( $zDateDeb, $zDateFin, $zTelecon, $lg_traitement['id_type_traitement'], ($lg_liste['libelle_categorie_grille']) , $zAuditeur, $zProjet, $zClient, $zApplication )."</td>
				</tr>
			";
		}
